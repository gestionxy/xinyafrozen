...
const HistoryDashboard: React.FC<HistoryDashboardProps> = ({ onEditSession }) => {
  ...
  const handleStartRename = (e: React.MouseEvent, session: HistorySession) => {
    e.stopPropagation();
    setEditingNameSessionId(session.id);
    setTempName(session.name || session.timestamp);
  };
  ...
  const handleSaveName = async (e: React.MouseEvent) => {
    ...
    try {
      await db.updateSessionName(editingNameSessionId, tempName);
                    ...
                    <div>
                      {editingNameSessionId === session.id ? (
                        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                          <input
                            type="text"
                            className="px-3 py-1 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-bold text-gray-900"
                            value={tempName}
                          ...
                          <button onClick={(e) => { e.stopPropagation(); setEditingNameSessionId(null); }} className="p-1 text-gray-400 hover:bg-gray-100 rounded">
                            <X size={18} />
                          </button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 group/title">
                          <h3 className="text-lg font-bold text-gray-900">{session.name || session.timestamp}</h3>
                          <button
                      ...
                      <p className="text-sm text-gray-500">{session.timestamp} • {session.orders.length} unique items ordered</p>
                    </div>
                  </div>
